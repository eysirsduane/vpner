// Just Swagger Frontend Application
// 版本号从后端注入
let JUST_SWAGGER_VERSION = '1.6.7';
...
function displayVersion() {
    ...
    if (footer) {
        footer.innerHTML = `Powered by Just Swagger · v${JUST_SWAGGER_VERSION}`;
    }
}
...
async function loadConfig() {
    try {
        ...
        // 注入版本号
        if (config.justSwaggerVersion) {
            JUST_SWAGGER_VERSION = config.justSwaggerVersion;
        }
        ...
        // 检查本地存储是否有主题设置
        const savedDarkMode = localStorage.getItem('just_swagger_dark_mode');
...
function saveCurrentEnvToStorage() {
    try {
        localStorage.setItem('just_swagger_current_env', currentEnvIndex.toString());
    } catch (e) {}
}
...
function loadCurrentEnvFromStorage() {
    try {
        const saved = localStorage.getItem('just_swagger_current_env');
        if (saved !== null) {
...
function saveGroupState(tagName, isExpanded) {
    try {
        const states = JSON.parse(sessionStorage.getItem('just_swagger_group_states') || '{}');
        states[tagName] = isExpanded;
        sessionStorage.setItem('just_swagger_group_states', JSON.stringify(states));
    } catch (e) {}
}
...
function renderParameters(api) {
    const params = (api.parameters || []).filter(p => p.in !== 'header');
...
function renderDebugPanel(api, path) {
    ...
    ensureGlobalHeaderParams(params);
    renderGlobalHeaders();
    
    // 获取保存的调试数据
    const savedData = getDebugData(currentApi.path, currentApi.method);
    
    const nonBodyParams = params.filter(p => p.in !== 'body' && p.in !== 'header');
...
function getDebugStorageKey(path, method) {
    return `just_swagger_debug_${method}_${path}`;
}
...
function loadBodyTemplates() {
    try {
        const saved = localStorage.getItem('just_swagger_body_templates');
...
function saveBodyTemplates() {
    try {
        localStorage.setItem('just_swagger_body_templates', JSON.stringify(bodyTemplates));
    } catch (e) {}
}
...
function ensureGlobalHeaderParams(params) {
    const headerParams = (params || []).filter(p => p.in === 'header' && p.name);
    if (headerParams.length === 0) return;

    let changed = false;
    headerParams.forEach(p => {
        const exists = globalHeaders.some(h => h.key && h.key.toLowerCase() === p.name.toLowerCase());
        if (!exists) {
            globalHeaders.push({ key: p.name, value: getHeaderParamDefaultValue(p) });
            changed = true;
        }
    });
...
function getHeaderParamDefaultValue(param) {
...
function renderGlobalHeaders() {
    ...
    const activeHeaders = globalHeaders.filter(h => h.key);
    ...
    if (activeHeaders.length > 0) {
        ...
        list.innerHTML = activeHeaders.map(h => `
            <div class="flex items-center gap-2 py-1">
                ...
                <span style="color: var(--text-secondary)">${h.value ? escapeHtml(maskValue(h.key, h.value)) : '未填写'}</span>
            </div>
        `).join('');
    } else {
...
function loadGlobalHeadersFromStorage() {
    try {
        const saved = localStorage.getItem('just_swagger_global_headers');
        if (saved) {
            globalHeaders = JSON.parse(saved);
        }
        mergeDefaultGlobalHeaders();
        updateHeadersCount();
    } catch (e) {
...
function mergeDefaultGlobalHeaders() {
    if (!Array.isArray(config.globalHeaders) || config.globalHeaders.length === 0) return;

    let changed = false;
    config.globalHeaders.forEach(h => {
        if (!h.key) return;
        const exists = globalHeaders.some(item => item.key && item.key.toLowerCase() === h.key.toLowerCase());
        if (!exists) {
            globalHeaders.push({ key: h.key, value: h.value || '' });
            changed = true;
        }
    });

    if (changed) saveGlobalHeadersToStorage();
}

function saveGlobalHeadersToStorage() {
    try {
        localStorage.setItem('just_swagger_global_headers', JSON.stringify(globalHeaders));
    } catch (e) {}
}

function updateHeadersCount() {
    const count = globalHeaders.filter(h => h.key).length;
...
function loadTokenExtractRulesFromStorage() {
    try {
        const saved = localStorage.getItem('just_swagger_token_rules');
        if (saved) tokenExtractRules = JSON.parse(saved);
        mergeDefaultTokenExtractRules();
        updateTokenRulesCount();
    } catch (e) {}
}

function mergeDefaultTokenExtractRules() {
    if (!Array.isArray(config.tokenExtractRules) || config.tokenExtractRules.length === 0) return;

    let changed = false;
    config.tokenExtractRules.forEach(rule => {
        if (!rule.jsonPath || !rule.headerKey) return;
        const exists = tokenExtractRules.some(item =>
            (item.pathPattern || '') === (rule.pathPattern || '') &&
            item.jsonPath === rule.jsonPath &&
            item.headerKey === rule.headerKey
        );
        if (!exists) {
    ...
    if (changed) saveTokenExtractRulesToStorage();
}

function saveTokenExtractRulesToStorage() {
    try {
        localStorage.setItem('just_swagger_token_rules', JSON.stringify(tokenExtractRules));
    } catch (e) {}
}
...
function loadThemeFromStorage() {
    try {
        const savedDarkMode = localStorage.getItem('just_swagger_dark_mode');
        const savedThemeColor = localStorage.getItem('just_swagger_theme_color');
...
function saveThemeToStorage() {
    try {
        localStorage.setItem('just_swagger_dark_mode', isDarkMode);
        localStorage.setItem('just_swagger_theme_color', currentThemeColor);
    } catch (e) {}
}
...
function loadUIThemeFromStorage() {
    try {
        const savedUITheme = localStorage.getItem('just_swagger_ui_theme');
...
function switchUITheme(theme) {
    // 保存到 localStorage
    localStorage.setItem('just_swagger_ui_theme', theme);
    const currentUrl = new URL(window.location.href);
