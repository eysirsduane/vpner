// Package justswagger provides a beautiful Swagger UI replacement for Go web frameworks
// Just Swagger - Just VPN API documentation
//
// 特性:
...
// Version is the current version of Just Swagger
const Version = "1.6.7"
...
// TokenExtractRule represents a response token extraction rule
type TokenExtractRule struct {
...
// Config holds the configuration for Just Swagger UI
// Just Swagger配置
type Config struct {
	...
	// TokenExtractRules are default response token extraction rules
	// 默认响应 Token 自动提取规则
	TokenExtractRules []TokenExtractRule
...
func DefaultConfig() Config {
	return Config{
		...
		Description: "API Documentation powered by Just Swagger",
		Version:     "1.0.0",
		BasePath:    "/doc",
		EnableDebug: true,
		DarkMode:    false,
...
// Handler returns a Gin handler for Just Swagger UI
...
// RegisterRoutes registers Just Swagger routes to a Gin router group
...
//
// 使用示例:
//   - 标准库: http.Handle("/doc/", justswagger.HTTPHandler(cfg))
//   - Echo: e.GET("/doc/*", echo.WrapHandler(justswagger.HTTPHandler(cfg)))
//   - Fiber: app.Use("/doc", adaptor.HTTPHandler(justswagger.HTTPHandler(cfg)))
//   - Chi: r.Handle("/doc/*", justswagger.HTTPHandler(cfg))
func HTTPHandler(cfg Config) http.Handler {
	...
	configJSON, _ := json.Marshal(map[string]interface{}{
		...
		"tokenExtractRules":  cfg.TokenExtractRules,
		"defaultTheme":       defaultTheme,
		"themes":             []string{"default", "minimal", "modern"},
		"justSwaggerVersion": Version,
		"logo":               cfg.Logo,
