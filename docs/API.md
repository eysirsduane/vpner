...
| --- | --- | --- |
| `200` | 请求成功 | 正常处理响应数据。 |
| `401` | 登录状态失效 | 清除旧 Token，重新调用自动登录接口 `/oldest/ointer/thisip/gstauth`，并保存返回的 `olds_est_orspd_olds_ast.olds_est_sesskey_olds_ast`。 |
...
1. 清除本地旧 Token。
2. 重新调用 `POST /oldest/ointer/thisip/gstauth`。
...
- `POST /oldest/ointer/thisip/tunfetch`
- `POST /oldest/ointer/thisip/tunexit`
- `POST /oldest/ointer/thisip/tunconf`
...
```http
GET /oldest/ointer/thisip/srvchk
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/srvchk:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/srvchk:end -->

### 获取静态资源

```http
GET /oldest/ointer/thisip/assetget/{filepath}
```
...
```text
/oldest/ointer/thisip/assetget/banner.png
/oldest/ointer/thisip/assetget/ad/home.png
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/assetget/*filepath:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/assetget/*filepath:end -->

### 游客登录

```http
POST /oldest/ointer/thisip/gstauth
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/gstauth:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/gstauth:end -->

### 注册账号

```http
POST /oldest/ointer/thisip/acctnew
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/acctnew:start -->
#### 请求字段说明
...
| `olds_est_orspd_olds_ast.olds_est_lastauth_olds_ast` | `string` | 成功时 | 本次登录时间 |
| `olds_est_orspd_olds_ast.olds_est_pwdtxt_olds_ast` | `string` | 成功时 | 绑定账号的明文密码，游客为空 |
<!-- endpoint-fields:/oldest/ointer/thisip/acctnew:end -->

### 账号登录

```http
POST /oldest/ointer/thisip/mbrauth
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/mbrauth:start -->
#### 请求字段说明
...
| `olds_est_orspd_olds_ast.olds_est_lastauth_olds_ast` | `string` | 成功时 | 本次登录时间 |
| `olds_est_orspd_olds_ast.olds_est_pwdtxt_olds_ast` | `string` | 成功时 | 绑定账号的明文密码，游客为空 |
<!-- endpoint-fields:/oldest/ointer/thisip/mbrauth:end -->
...
```http
POST /oldest/ointer/thisip/sessbye
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/sessbye:start -->
#### 请求字段说明
...
| `olds_est_orspd_olds_ast.olds_est_lastauth_olds_ast` | `string` | 成功时 | 本次登录时间 |
| `olds_est_orspd_olds_ast.olds_est_pwdtxt_olds_ast` | `string` | 成功时 | 绑定账号的明文密码，游客为空 |
<!-- endpoint-fields:/oldest/ointer/thisip/sessbye:end -->

### 账号注销

```http
POST /oldest/ointer/thisip/acctoff
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/acctoff:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/acctoff:end -->

### 修改密码

```http
POST /oldest/ointer/thisip/secrnw
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/secrnw:start -->
#### 请求字段说明
...
| `olds_est_orspm_olds_ast` | `string` | 是 | 业务提示信息；失败时前端可按产品交互展示该文案。 |
| `olds_est_orspd_olds_ast` | `object` | 是 | 接口业务数据；具体结构见下方以该字段开头的嵌套字段。 |
<!-- endpoint-fields:/oldest/ointer/thisip/secrnw:end -->

### 获取设备列表

```http
POST /oldest/ointer/thisip/devmeta
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/devmeta:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/devmeta:end -->

### 移除设备

```http
POST /oldest/ointer/thisip/devexit
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/devexit:start -->
#### 请求字段说明
...
| `olds_est_orspd_olds_ast.olds_est_devlst_olds_ast[].olds_est_relver_olds_ast` | `string` | 成功时 | 客户端展示版本号，例如 `1.0.0`。 |
| `olds_est_orspd_olds_ast.olds_est_devlst_olds_ast[].olds_est_lastauth_olds_ast` | `string` | 成功时 | 设备或用户最近登录时间，格式为 `yyyy-MM-dd HH:mm:ss`。 |
<!-- endpoint-fields:/oldest/ointer/thisip/devexit:end -->

### 获取邀请码

```http
POST /oldest/ointer/thisip/refmk
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/refmk:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/refmk:end -->

### 填写邀请码

```http
POST /oldest/ointer/thisip/refbind
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/refbind:start -->
#### 请求字段说明
...
| `olds_est_orspm_olds_ast` | `string` | 是 | 业务提示信息；失败时前端可按产品交互展示该文案。 |
| `olds_est_orspd_olds_ast` | `object` | 是 | 接口业务数据；具体结构见下方以该字段开头的嵌套字段。 |
<!-- endpoint-fields:/oldest/ointer/thisip/refbind:end -->

### 邀请详情

```http
POST /oldest/ointer/thisip/refdtl
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/refdtl:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/refdtl:end -->

### 用户信息

```http
GET /oldest/ointer/thisip/mbrmeta
```

响应 `olds_est_orspd_olds_ast` 为用户信息。

<!-- endpoint-fields:/oldest/ointer/thisip/mbrmeta:start -->
#### 请求字段说明
...
| `olds_est_orspd_olds_ast.olds_est_lastauth_olds_ast` | `string` | 成功时 | 本次登录时间 |
| `olds_est_orspd_olds_ast.olds_est_pwdtxt_olds_ast` | `string` | 成功时 | 绑定账号的明文密码，游客为空 |
<!-- endpoint-fields:/oldest/ointer/thisip/mbrmeta:end -->
...
```http
POST /oldest/ointer/thisip/initopts
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/initopts:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/initopts:end -->

### 版本检测

```http
POST /oldest/ointer/thisip/relchk
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/relchk:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/relchk:end -->

### 广告位列表

```http
GET /oldest/ointer/thisip/promolst
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/promolst:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/promolst:end -->

### 全量通知

```http
GET /oldest/ointer/thisip/msgbox
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/msgbox:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/msgbox:end -->

### 未读通知

```http
GET /oldest/ointer/thisip/msgunrd
```

响应结构同全量通知，只返回未读消息。

<!-- endpoint-fields:/oldest/ointer/thisip/msgunrd:start -->
#### 请求字段说明
...
| `olds_est_orspd_olds_ast.olds_est_usrmsg_olds_ast.olds_est_totalcnt_olds_ast` | `integer` | 成功时 | 当前查询结果的总数量。 |
| `olds_est_orspd_olds_ast.olds_est_usrmsg_olds_ast.olds_est_unreadcnt_olds_ast` | `integer` | 成功时 | 当前未读通知总数。 |
<!-- endpoint-fields:/oldest/ointer/thisip/msgunrd:end -->

### 获取弹窗

```http
GET /oldest/ointer/thisip/dlgshow
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/dlgshow:start -->
#### 请求字段说明
...
| `olds_est_orspd_olds_ast.olds_est_dispcnt_olds_ast` | `integer` | 成功时 | 当前用户已展示次数 |
| `olds_est_orspd_olds_ast.olds_est_maxdispcnt_olds_ast` | `integer` | 成功时 | 每个用户最大展示次数，0表示不限次数 |
<!-- endpoint-fields:/oldest/ointer/thisip/dlgshow:end -->

### 延迟迁移弹窗

```http
GET /oldest/ointer/thisip/dlgdelay
```
...
```json
{
    ...
    "olds_est_orspd_olds_ast":  {
                           ...
                           "olds_est_imgaddr_olds_ast":  [
                                                   "/oldest/ointer/thisip/assetget/pop.png"
                                               ],
...
<!-- endpoint-fields:/oldest/ointer/thisip/dlgdelay:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/dlgdelay:end -->

### 标记通知已读

```http
POST /oldest/ointer/thisip/msgack
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/msgack:start -->
#### 请求字段说明
...
| `olds_est_orspm_olds_ast` | `string` | 是 | 业务提示信息；失败时前端可按产品交互展示该文案。 |
| `olds_est_orspd_olds_ast` | `object` | 是 | 接口业务数据；具体结构见下方以该字段开头的嵌套字段。 |
<!-- endpoint-fields:/oldest/ointer/thisip/msgack:end -->
...
```http
POST /oldest/ointer/thisip/faultrpt
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/faultrpt:start -->
#### 请求字段说明
...
| `olds_est_orspm_olds_ast` | `string` | 是 | 业务提示信息；失败时前端可按产品交互展示该文案。 |
| `olds_est_orspd_olds_ast` | `object` | 是 | 接口业务数据；具体结构见下方以该字段开头的嵌套字段。 |
<!-- endpoint-fields:/oldest/ointer/thisip/faultrpt:end -->
...
```http
GET /oldest/ointer/thisip/offerlst
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/offerlst:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/offerlst:end -->
...
```http
GET /oldest/ointer/thisip/shrapl
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/shrapl:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/shrapl:end -->

### 发起支付

```http
POST /oldest/ointer/thisip/billstart
```
...
| 验单      | 支付完成后调用 `POST /oldest/ointer/thisip/aplverify`，只提交 `olds_est_txnid_olds_ast` |
...
<!-- endpoint-fields:/oldest/ointer/thisip/billstart:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/billstart:end -->

### 苹果订单验证

```http
POST /oldest/ointer/thisip/aplverify
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/aplverify:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/aplverify:end -->
...
```http
POST /oldest/ointer/thisip/reglst
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/reglst:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/reglst:end -->
...
| 接口 | 返回内容 | 状态 |
| --- | --- | --- |
| `POST /oldest/ointer/thisip/tunfetch` | 加密节点 URL | 母版和现有 VPN 项目已实现 |
| `POST /oldest/ointer/thisip/tunconf` | 加密的完整 JSON 配置 | 母版和大部分项目已实现，少数老项目尚无 |
| `POST /oldest/ointer/thisip/tunexit` | 分别加密的节点 URL 和 `olds_est_outlst_olds_ast` JSON | 已实现 |
...
<!-- endpoint-fields:/oldest/ointer/thisip/tunfetch:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/tunfetch:end -->
...
| `olds_est_lnkuri_olds_ast` | string | 必返 | 与 `/oldest/ointer/thisip/tunfetch` 相同的加密节点 URL；先做标准 Base64 解码，再执行 AES-CBC/PKCS7 解密，最后再次标准 Base64 解码 |
| `olds_est_outlst_olds_ast` | string | 必返 | 独立加密的 JSON 数组，必须与 `olds_est_lnkuri_olds_ast` 分别解密；解密方式相同，解密后与 `/oldest/ointer/thisip/tunconf` 完整配置中的 `olds_est_outlst_olds_ast` 完全一致 |

<!-- endpoint-fields:/oldest/ointer/thisip/tunexit:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/tunexit:end -->
...
`olds_est_cfgdata_olds_ast` 是完整 JSON 节点配置的加密结果，解密流程与 `olds_est_lnkuri_olds_ast` 相同：先对返回字符串做标准 Base64 解码得到 AES 密文，再以 AES-CBC/PKCS7 解密得到内部 Base64 字符串，最后再次标准 Base64 解码得到 JSON 配置。该接口与 `/oldest/ointer/thisip/tunfetch` 使用相同的线路选择、会员校验和临时节点记录逻辑。

<!-- endpoint-fields:/oldest/ointer/thisip/tunconf:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/tunconf:end -->

### 确认已连接

```http
POST /oldest/ointer/thisip/connup
```

客户端成功建立 VPN 后调用。

<!-- endpoint-fields:/oldest/ointer/thisip/connup:start -->
#### 请求字段说明
...
| `olds_est_orspm_olds_ast` | `string` | 是 | 业务提示信息；失败时前端可按产品交互展示该文案。 |
| `olds_est_orspd_olds_ast` | `object` | 是 | 接口业务数据；具体结构见下方以该字段开头的嵌套字段。 |
<!-- endpoint-fields:/oldest/ointer/thisip/connup:end -->

### 心跳

```http
POST /oldest/ointer/thisip/connbeat
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/connbeat:start -->
#### 请求字段说明
...
<!-- endpoint-fields:/oldest/ointer/thisip/connbeat:end -->

### 断开连接

```http
POST /oldest/ointer/thisip/conndown
```

主动断开 VPN 时调用。

<!-- endpoint-fields:/oldest/ointer/thisip/conndown:start -->
#### 请求字段说明
...
| `olds_est_orspm_olds_ast` | `string` | 是 | 业务提示信息；失败时前端可按产品交互展示该文案。 |
| `olds_est_orspd_olds_ast` | `object` | 是 | 接口业务数据；具体结构见下方以该字段开头的嵌套字段。 |
<!-- endpoint-fields:/oldest/ointer/thisip/conndown:end -->

### 流量上报

```http
POST /oldest/ointer/thisip/trfrpt
```
...
<!-- endpoint-fields:/oldest/ointer/thisip/trfrpt:start -->
#### 请求字段说明
...
| `olds_est_orspm_olds_ast` | `string` | 是 | 业务提示信息；失败时前端可按产品交互展示该文案。 |
| `olds_est_orspd_olds_ast` | `object` | 是 | 接口业务数据；具体结构见下方以该字段开头的嵌套字段。 |
<!-- endpoint-fields:/oldest/ointer/thisip/trfrpt:end -->
...
| --- | --- | --- | --- |
| 建立登录态 | 应用首次安装、本地没有 Token，或鉴权接口返回业务码 `401` 时，调用 `POST /oldest/ointer/thisip/gstauth`。请求体至少传 `olds_est_devnum_olds_ast`；平台、版本、设备名称、系统版本、设备型号和来源渠道按该接口字段表传递。 | 保存 `olds_est_orspd_olds_ast.olds_est_sesskey_olds_ast`，并缓存 `olds_est_orspd_olds_ast.olds_est_recid_olds_ast`、`olds_est_orspd_olds_ast.olds_est_vipflg_olds_ast`、`olds_est_orspd_olds_ast.olds_est_vipexptm_olds_ast`。同一安装应始终使用稳定的设备号，避免重复创建游客。 | 自动登录本身失败时展示业务提示或网络重试；不要拿已经收到 `401` 的旧 Token 循环重试原接口。 |
| 获取运营配置 | 已取得 Token 后调用 `POST /oldest/ointer/thisip/initopts`。 | 缓存协议、分享、邀请、官网、试用时长、客服入口、工具开关和代理白名单。后端配置可热更新，因此每次冷启动应重新获取。 | 单个可选配置为空时使用客户端默认值，不应把空配置当成登录失败。 |
| 检查版本 | 调用 `POST /oldest/ointer/thisip/relchk`，构建号从请求头 `Oldests-Building-Version` 读取。 | 根据返回的是否更新、是否强制、版本号、下载地址、大小和更新内容决定是否展示更新界面。 | 没有匹配的更新版本时返回空结果属于正常情况；强制更新时应阻止继续进入主界面。 |
| 获取首页内容 | 按页面需要调用 `GET /oldest/ointer/thisip/dlgshow`、`GET /oldest/ointer/thisip/dlgdelay`、`GET /oldest/ointer/thisip/promolst` 和 `GET /oldest/ointer/thisip/msgunrd`。 | 普通弹窗按后端返回直接展示；延迟弹窗按返回延迟控制；未读数用于消息角标；广告按启用状态和展示位置渲染。 | 返回空对象或空数组通常表示当前没有可展示内容，不应提示系统错误。弹窗展示次数和新用户、地区限制由后端判断。 |
...
进入线路页面后调用 `POST /oldest/ointer/thisip/reglst`。使用 `olds_est_orspd_olds_ast[].olds_est_ctrynm_olds_ast` 展示线路名称，提交获取节点请求时必须原样使用同一项的 `olds_est_orspd_olds_ast[].olds_est_regcd_olds_ast`，例如 `HK` 或 `AUTO`，不能根据展示文案自行拼接代码。

客户端应根据自身实现固定选择下面一种节点接口；一次正常连接不需要把三种接口全部调用一遍。三种接口共用会员校验、审核版本、试用节点和负载均衡逻辑，区别只在返回格式。

| 客户端能力 | 请求接口与请求体 | 成功响应的使用方式 |
| --- | --- | --- |
| 客户端自行拼接完整配置 | 调用 `POST /oldest/ointer/thisip/tunfetch`，请求体传 `olds_est_rtecd_olds_ast`。 | 解密 `olds_est_orspd_olds_ast.olds_est_lnkuri_olds_ast` 得到原始节点 URL，再由客户端生成完整配置。 |
| 客户端只拼接配置其余部分 | 调用 `POST /oldest/ointer/thisip/tunexit`，请求体传 `olds_est_rtecd_olds_ast`。 | 分别解密 `olds_est_orspd_olds_ast.olds_est_lnkuri_olds_ast` 和 `olds_est_orspd_olds_ast.olds_est_outlst_olds_ast`；后者是可直接嵌入配置的 `olds_est_outlst_olds_ast` JSON 数组。 |
| 客户端直接使用完整 JSON | 调用 `POST /oldest/ointer/thisip/tunconf`，请求体传 `olds_est_rtecd_olds_ast` 和 `olds_est_cfgtyp_olds_ast`；连接模式使用 `fast`/`极速` 或 `global`/`全局`。 | 解密 `olds_est_orspd_olds_ast.olds_est_cfgdata_olds_ast` 得到完整 JSON 配置并交给连接内核。 |
...
| --- | --- | --- |
| VPN 内核尚未确认连通 | 暂时不要调用 `POST /oldest/ointer/thisip/connup`。如果内核直接失败，可按产品交互调用 `POST /oldest/ointer/thisip/faultrpt` 上报错误信息。 | 获取到节点不等于连接成功，不能提前产生在线记录。 |
| VPN 内核已确认连通 | 立即调用 `POST /oldest/ointer/thisip/connup`。 | 业务码 `200` 后开始心跳；如果该上报因瞬时网络问题失败，只要会员仍有效，后续心跳没有在线记录时也会返回继续连接。 |
| VPN 正在连接 | 每 30 秒调用 `POST /oldest/ointer/thisip/connbeat`。 | `olds_est_orspd_olds_ast.olds_est_connst_olds_ast=1` 时保持连接；`olds_est_orspd_olds_ast.olds_est_connst_olds_ast=0` 时立即让内核断开。当前逻辑只有会员失效才要求断开。 |
| 用户主动断开或内核结束 | 调用 `POST /oldest/ointer/thisip/conndown`，并停止心跳定时器。 | 即使断开上报失败，也必须先清理客户端本地连接状态，后续可在有网络时记录错误。 |
| 产生可统计流量 | 在适当的周期或断开前调用 `POST /oldest/ointer/thisip/trfrpt`，请求体 `olds_est_trfvol_olds_ast` 传本次增量流量。 | 单位为 K，值必须大于 `0`；不要重复累计上报同一段流量。 |

### 套餐购买、苹果验单与会员状态刷新

| 阶段 | 请求与判断 | 前端处理 |
| --- | --- | --- |
| 展示套餐 | 调用 `GET /oldest/ointer/thisip/offerlst`。 | 使用返回数组渲染套餐；用户选择后保存该项 `olds_est_orspd_olds_ast[].olds_est_recid_olds_ast`。发起支付时只能提交接口实际返回且当前上架的套餐 ID，不能把数组下标当套餐 ID。 |
| 创建订单并决定支付渠道 | 调用 `POST /oldest/ointer/thisip/billstart`，请求体 `olds_est_pkgid_olds_ast` 传选中的套餐 ID。支付渠道由后端根据时区、IP 地区、版本和支付配置决定。 | 读取 `olds_est_orspd_olds_ast.olds_est_catid_olds_ast`，不要由客户端自行判断走苹果还是 H5。 |
| 返回 `apple_iap` | `olds_est_orspd_olds_ast.olds_est_destaddr_olds_ast` 是 Apple 商品 ID，`olds_est_orspd_olds_ast.olds_est_appusrtkn_olds_ast` 是本次后端订单 UUID。 | 使用商品 ID 调起 StoreKit，并把订单 UUID 原样作为 `appAccountToken`；支付成功取得苹果交易号后调用 `POST /oldest/ointer/thisip/aplverify`，请求体传 `olds_est_txnid_olds_ast`。验单业务码 `200` 后，以 `olds_est_orspd_olds_ast.olds_est_vipexptm_olds_ast` 更新界面，并再调用用户信息接口校准会员状态。 |
| 返回 `h5` | `olds_est_orspd_olds_ast.olds_est_destaddr_olds_ast` 是三方支付页面 URL，`olds_est_orspd_olds_ast.olds_est_appusrtkn_olds_ast` 为空。 | 用系统浏览器或约定 WebView 打开 URL。支付成功由三方服务端回调后端发放会员，客户端不要调用支付回调接口；用户返回 App 后调用 `GET /oldest/ointer/thisip/mbrmeta` 刷新 `olds_est_orspd_olds_ast.olds_est_vipflg_olds_ast` 和 `olds_est_orspd_olds_ast.olds_est_vipexptm_olds_ast`。 |
...
| --- | --- | --- |
| 游客首次进入 | 调用 `POST /oldest/ointer/thisip/gstauth`，设备号使用稳定值。 | 保存 `olds_est_orspd_olds_ast.olds_est_sesskey_olds_ast`。后端可能创建新游客，也可能找到已有设备用户；前端不需要区分创建接口。 |
| 游客注册账号 | 在已有游客 Token 下调用 `POST /oldest/ointer/thisip/acctnew`，提交 `olds_est_devnum_olds_ast`、`olds_est_usrnm_olds_ast`、`olds_est_pwdtxt_olds_ast`。账号和密码均为 6–20 位字母或数字。 | 业务码 `200` 后必须用响应中的 `olds_est_orspd_olds_ast.olds_est_sesskey_olds_ast` 覆盖本地 Token，并刷新用户类型、账号名和会员状态。 |
| 已有账号登录当前设备 | 调用 `POST /oldest/ointer/thisip/mbrauth`，至少提交 `olds_est_devnum_olds_ast`、`olds_est_usrnm_olds_ast`、`olds_est_pwdtxt_olds_ast`，其余设备环境字段按接口字段表传递。 | 业务码 `200` 后用 `olds_est_orspd_olds_ast.olds_est_sesskey_olds_ast` 覆盖本地 Token。账号不存在、密码错误、设备数达到上限等返回 `500`，直接展示提示。 |
| 刷新个人资料 | 调用 `GET /oldest/ointer/thisip/mbrmeta`。 | 使用服务端返回覆盖会员状态和到期时间；支持 App Store 地区或语言记录的产品也会在此接口按非空有效请求头更新用户资料。 |
| 查看或移除其他设备 | 调用 `POST /oldest/ointer/thisip/devmeta` 获取本机及其他设备；移除设备时调用 `POST /oldest/ointer/thisip/devexit`，用 `olds_est_recid_olds_ast` 提交设备列表返回的记录 ID。 | 不能提交用户 ID 或设备号代替设备记录 ID。移除操作会把目标设备恢复成游客并清除该设备会员；目标设备的原 Token 仍可继续鉴权，但后续读取到的是游客状态。 |
| 当前设备退出账号 | 调用 `POST /oldest/ointer/thisip/sessbye`。 | 当前设备恢复游客状态并清空本设备会员，账号会员仍保留；保存接口返回的新用户信息和 Token。不要把“退出账号”当成删除账号。 |
| 修改密码 | 已登录账号调用 `POST /oldest/ointer/thisip/secrnw`，只提交 `olds_est_newpwd_olds_ast`，无需旧密码。 | 新密码必须为 6–20 位字母或数字且不能与当前密码相同；成功后 Token 不需要更换。 |
| 注销账号 | 调用 `POST /oldest/ointer/thisip/acctoff`。是否真实注销由请求头 `Oldests-Number` 是否命中服务端配置决定。 | 真实注销会删除账号或设备用户并创建新游客，必须保存 `olds_est_orspd_olds_ast.olds_est_sesskey_olds_ast`；未命中真实注销版本时返回成功空对象，原 Token 继续有效。前端必须兼容这两种成功响应。 |

### 通知读取、邀请和客户端错误上报

通知页面先调用 `GET /oldest/ointer/thisip/msgbox` 获取系统与个人通知；用户打开通知后调用 `POST /oldest/ointer/thisip/msgack`，分别用 `olds_est_sysmsgids_olds_ast` 和 `olds_est_usrmsgids_olds_ast` 提交系统通知 ID 数组与个人通知 ID 数组。读取成功后再更新本地已读状态和角标，避免网络失败时界面与服务端不一致。

邀请页面调用 `POST /oldest/ointer/thisip/refmk` 获取自己的邀请码，输入他人邀请码时调用 `POST /oldest/ointer/thisip/refbind` 并将输入值放入 `olds_est_refcd_olds_ast`；邀请记录和奖励进度从 `POST /oldest/ointer/thisip/refdtl` 获取。当前用户是否仍可填写他人邀请码及奖励是否到账以接口返回为准，不由客户端本地计算。

VPN 内核、配置解析或其他可诊断错误可调用 `POST /oldest/ointer/thisip/faultrpt` 上报，用 `olds_est_errtyp_olds_ast` 标识错误分类，用 `olds_est_msgtxt_olds_ast` 提交可诊断信息，并同时提交 `olds_est_osver_olds_ast` 操作系统版本和 `olds_est_mdlnm_olds_ast` 设备型号。上报内容不得包含登录密码、支付密钥、完整 Token、节点解密密钥或其他敏感信息；错误上报失败也不能阻塞用户退出连接或继续使用 App。
