...
- 使用 Redis `SET NX` 原子占用参与次数，键为 `lucky_wheel:played:<user_id>`，TTL 为 `lucky_wheel.round_time` 配置的秒数，从参与时刻起计算，不在零点重置。默认 `86400` 秒；配置缺失、非正整数或超出可表示时长时使用默认值。例如配置 `3600`，参与后需等待一小时。已有未过期标记则拒绝参与；Redis 不可用时返回错误，不继续抽奖。
- 不再查询数据库中的参与记录。抽奖或记录写入失败时，通过比较请求标识的 Lua 脚本释放本次占位；写入成功后保留标记至过期。Redis 标记被清空或淘汰后可能重复参与；旧数据库记录不会自动回填 Redis。进程中断或占位释放失败时，标记可能保留至配置时长结束。配置变更用于新写入的标记，不修改已存在标记的 TTL；旧版日期键不会自动迁移。
...
两个参与条件均不满足时返回业务状态码 `500`，提示“当前用户暂不可参与幸运转盘”，不会占用本轮次数。Redis 中存在未过期参与标记时返回业务状态码 `500`，提示“您已参与本轮幸运转盘，请冷却结束后再试”；删除数据库参与记录不会清除 Redis 标记。无可用记录、Redis 占位失败、数据库查询或记录写入失败时，返回业务状态码 `500`；未登录返回 `401`，HTTP 状态码均为 `200`。本接口保存中奖信息，不修改用户会员时长。
...
- `today_played` 保留原字段名，根据当前 Redis 参与标记是否存在返回，表示用户是否仍在配置的参与冷却期内，与抽奖接口一致；正在抽奖的占位也视为已参与。只查询，不写入或延长 TTL。Redis 读取失败返回业务状态码 `500`。
...
| 实际字段 | 原始字段 | 类型 | 说明 |
| --- | --- | --- | --- |
| `mid_call_tdpl_call_fix` | `today_played` | boolean | 存在未过期参与标记为 `true`，否则为 `false`，跨零点不重置 |
...
- 鉴权：`Authorization: Bearer <token>`，无需请求参数或请求体。
- 事务前读取本轮参与标识并使用 Redis `SET NX` 原子占位，键为 `lucky_wheel:claimed:<user_id>:<参与标识>`，TTL 为 `lucky_wheel.round_time` 配置的秒数。不同轮次使用不同键，上一轮的标记不阻塞下一轮；参与标记缺失时使用 `lucky_wheel:claimed:<user_id>`，由数据库确认是否有有效记录。已有标记时直接返回业务状态码 `500`，提示“幸运转盘奖励正在领取或已领取，请勿重复操作”，不访问数据库，也不刷新 TTL。
- 事务提交成功或数据库确认已领取时保留标记；其他失败通过比较请求标识的 Lua 脚本释放本次占位。进程中断或释放失败可能导致标记保留至配置时长结束。Redis 不可用时回退数据库，仍保留领取状态校验、行锁和事务，避免缓存丢失后重复发奖。
- 以北京时间当前时刻（秒精度）为上界，查询 `(当前时间 - 配置时长, 当前时间]` 内当前用户的参与记录，按 `create_time DESC, id DESC` 取最近一条；不按领取状态过滤，避免重复请求领取更早记录。超出时间范围的记录无法领取。抽奖时已校验开关与用户类型条件，领奖时根据已保存的参与记录发放奖励。
...
- 成功返回业务状态码 `200`、消息 `success`，`mid_call_rslt_call_fix` 为空对象。配置周期内无记录、已领取、记录已删除或奖励时长无效时返回业务状态码 `500`。
